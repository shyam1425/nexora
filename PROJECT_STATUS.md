...
| Local Git baseline | PASS — `main` advanced from the initial commit `577f6d8` through the deployment records to the current tip, 217 tracked files, clean working tree, no secrets in the index or anywhere in the history |
...
| Production database (Aiven MySQL) | PARTIAL — the Aiven for MySQL 8 service is provisioned and reachable (`nexora-mysql-shyam-1209.k.aivencloud.com:18737`, database `defaultdb`, user `avnadmin`, CA at `certs/aiven-ca.pem`). Transport verified: DNS resolves, TCP 18737 connects, and with `ssl: { ca }` plus a deliberately incorrect password the driver reaches authentication (`ER_ACCESS_DENIED_ERROR 1045`), proving TLS negotiation and Aiven Project CA verification both succeeded; without TLS the attempt fails with `ER_CANNOT_RETRIEVE_RSA_KEY (45044)`. **No successful query yet** — the database password is not present in this workspace, so no authenticated connection and no migration have been performed |
...
- The production Aiven MySQL service exists (`nexora-mysql-shyam-1209.k.aivencloud.com:18737`, database `defaultdb`, user `avnadmin`, Project CA at `certs/aiven-ca.pem`), but its password is not available anywhere in this workspace: there is no `.env.production.local`, no `DATABASE_URL` in the process/user/machine environment, no Windows Credential Manager entry, no Aiven CLI configuration, and no file in the repository that references the host. Because of that, no authenticated connection and no migration have been run, and the Vercel deployment still returns 503 from `/api/v1/health` and fails every database-backed route. No credential was fabricated, guessed, or substituted to make a check pass.
...
- Database-backed public routes degrade to Next.js's generic error shell while the database is unreachable: the deployment log records `prisma.job.findMany()` → `P2028 Transaction API error: Unable to start a transaction in the given time` for `GET /careers`, and `prisma.$queryRaw()` → pool timeout `45028` for `GET /api/v1/health`. The same routes return 200 locally against a reachable database, so this is an availability consequence of the missing database rather than a logic defect; catching the failure and rendering an outage/empty state on public pages is an optional follow-up hardening task.
